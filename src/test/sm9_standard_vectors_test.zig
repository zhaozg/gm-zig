...
test "SM9 signature standard test vectors" {
    ...
    for (test_cases) |test_case| {
        ...
        // Sign message
        const signature = try signature_context.sign(test_case.message, user_key, sm9.sign.SignatureOptions{});

        // Verify signature
        const is_valid = try signature_context.verify(test_case.message, signature, test_case.user_id, sm9.sign.SignatureOptions{});
...
test "SM9 encryption standard test vectors" {
    ...
    for (test_cases) |test_case| {
        ...
        // Encrypt message
        const ciphertext = try encryption_context.encrypt(test_case.message, test_case.user_id, sm9.encrypt.EncryptionOptions{});

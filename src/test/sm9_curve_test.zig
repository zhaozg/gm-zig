...
test "SM9 Curve Operations - G1 Point Arithmetic" {
    ...
    const test_x = [_]u8{0x01} ++ @as([31]u8, @splat(0));
    const test_y = [_]u8{0x02} ++ @as([31]u8, @splat(0));
    ...
    const test_x2 = [_]u8{0x03} ++ @as([31]u8, @splat(0));
    const test_y2 = [_]u8{0x04} ++ @as([31]u8, @splat(0));
    ...
    const scalar = @as([31]u8, @splat(0)) ++ [_]u8{3};
...
test "SM9 Curve Operations - G2 Point Creation and Validation" {
    ...
    const x = [_]u8{0x01} ++ @as([63]u8, @splat(0));
    const y = [_]u8{0x02} ++ @as([63]u8, @splat(0));
...
test "SM9 Curve Operations - G2 Point Uncompressed Format" {
    const x = [_]u8{0x01} ++ @as([31]u8, @splat(0)); // 32 bytes
    const y = [_]u8{0x02} ++ @as([31]u8, @splat(0)); // 32 bytes

    // Create uncompressed format (65 bytes total)
    var uncompressed = [_]u8{0x04} ++ @as([64]u8, @splat(0));
    ...
    const inf_uncompressed = [_]u8{0x00} ++ @as([64]u8, @splat(0));
    const inf_point = try sm9.curve.G2Point.fromUncompressed(inf_uncompressed);
    try testing.expect(inf_point.isInfinity());

    // Test invalid format
    const invalid_uncompressed = [_]u8{0x05} ++ @as([64]u8, @splat(0)); // Invalid prefix
...
test "SM9 Curve Operations - G2 Point Arithmetic" {
    ...
    const x1 = [_]u8{0x01} ++ @as([63]u8, @splat(0));
    const y1 = [_]u8{0x02} ++ @as([63]u8, @splat(0));
...
test "SM9 Curve Operations - Edge Cases" {
    ...
    const scalar = @as([31]u8, @splat(0)) ++ [_]u8{5};
...
test "SM9 Curve Operations - Point Validation Edge Cases" {
    ...
    const x_valid = [_]u8{0x01} ++ @as([63]u8, @splat(0));
    const y_valid = [_]u8{0x02} ++ @as([63]u8, @splat(0));

...
pub const SM4 = struct {
    ...
    pub fn init(key: *const [SM4_KEY_SIZE]u8) SM4 {
        ...
        // 加载密钥
        k[0] = mem.readInt(u32, key[0..4], .big) ^ FK[0];
        k[1] = mem.readInt(u32, key[4..8], .big) ^ FK[1];
        k[2] = mem.readInt(u32, key[8..12], .big) ^ FK[2];
        k[3] = mem.readInt(u32, key[12..16], .big) ^ FK[3];
    ...
    pub fn encryptBlock(ctx: *const SM4, input: *const [SM4_BLOCK_SIZE]u8, output: *[SM4_BLOCK_SIZE]u8) void {
        var x0 = mem.readInt(u32, input[0..4], .big);
        var x1 = mem.readInt(u32, input[4..8], .big);
        var x2 = mem.readInt(u32, input[8..12], .big);
        var x3 = mem.readInt(u32, input[12..16], .big);

        inline for (0..8) |i| {
            const base = i * 4;
            x0 ^= sm4_T(x1 ^ x2 ^ x3 ^ ctx.rk[base]);
            x1 ^= sm4_T(x0 ^ x2 ^ x3 ^ ctx.rk[base + 1]);
            x2 ^= sm4_T(x0 ^ x1 ^ x3 ^ ctx.rk[base + 2]);
            x3 ^= sm4_T(x0 ^ x1 ^ x2 ^ ctx.rk[base + 3]);
        }

        mem.writeInt(u32, output[0..4], x3, .big);
        mem.writeInt(u32, output[4..8], x2, .big);
        mem.writeInt(u32, output[8..12], x1, .big);
    ...
    pub fn decryptBlock(ctx: *const SM4, input: *const [SM4_BLOCK_SIZE]u8, output: *[SM4_BLOCK_SIZE]u8) void {
        ...
        while (i > 0) {
            i -= 4;
            x0 ^= sm4_T(x1 ^ x2 ^ x3 ^ ctx.rk[i]);
            x1 ^= sm4_T(x0 ^ x2 ^ x3 ^ ctx.rk[i + 1]);
            x2 ^= sm4_T(x0 ^ x1 ^ x3 ^ ctx.rk[i + 2]);
            x3 ^= sm4_T(x0 ^ x1 ^ x2 ^ ctx.rk[i + 3]);
        }
    ...
    // S盒变换
    fn tau(a: u32) u32 {
        return (@as(u32, SBOX_T[@as(u8, @truncate(a >> 24))]) << 24) |
               (@as(u32, SBOX_T[@as(u8, @truncate(a >> 16))]) << 16) |
               (@as(u32, SBOX_T[@as(u8, @truncate(a >> 8))]) << 8) |
               (@as(u32, SBOX_T[@as(u8, @truncate(a))]));
    }

...
pub fn decodeSignatureDER(der_sig: []const u8) !struct { r: [32]u8, s: [32]u8 } {
    ...
    // Check SEQUENCE tag
    if (der_sig[pos] != 0x30) return error.InvalidDERSignature;
    pos += 1;
    
    // Read sequence length
    const seq_len = der_sig[pos];
    pos += 1;
    
    if (pos + seq_len != der_sig.len) return error.InvalidDERSignature;
    ...
    // Decode r
    const r = try decodeInteger(der_sig, &pos);
    
    // Decode s
    const s = try decodeInteger(der_sig, &pos);
